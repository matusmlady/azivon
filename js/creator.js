...
function exportSetup(){
  ...
  const d = new Date()
  document.getElementById('fileExport').download = 'azivon-' + d.getFullYear() + '-' + (d.getMonth() + 1) + '-' + d.getDate() + '-' + d.getHours() + '-' + d.getMinutes() + '-' + d.getSeconds() + '.json'
...
//todo auto fill not working not sure if we want to
//d.columns = Number(document.getElementById('columns').value)
//d.rows = Number(document.getElementById('rows').value)
generateMap(d.columns, d.rows)
